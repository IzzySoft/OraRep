...
# ====================================================[ Script starts here ]===
#cat $SQLSET $BINDIR/rephead.pls $PLUGINDIR/formatting.pls $SYSSTATFUNCS $USERHEAD $DBLINKHEAD $WAITHEAD $ENQHEAD $SPADVHEAD $RSRCHEAD $PROFHEAD $TSQUOTHEAD $TABSHEAD $FILESHEAD $DBWRHEAD $LGWRHEAD $RBSHEAD $MEMVALHEAD $POOLHEAD $BUFFRATHEAD $FLCHEAD $INVOBJHEAD $BINDIR/repopen.pls $INSTEFF $USERBODY $DBLINKBODY $RSRCBODY $PROFBODY $TSQUOTBODY $TABSBODY $FILESBODY $DBWRBODY $LGWRBODY $RBSBODY $MEMVALBODY $POOLBODY $TMPADV $SYSSTAT $WTEVTBODY $BUFFRATBODY $FLCBODY $WAITBODY $ENQBODY $INVOBJBODY $BINDIR/repclose.pls >rep.out
