...
if [ $MK_WTEVT -eq 1 ]; then
  WTEVTHEAD=$PLUGINDIR/wtevt_head.pls
...
if [ "${MK_TSCAN}${MK_NEXT}" != "00" ]; then
  MISCHEAD=$PLUGINDIR/misc_head.pls
  MISCBODY=$PLUGINDIR/misc_body.pls
fi
...
# ====================================================[ Script starts here ]===
#cat $SQLSET $BINDIR/rephead.pls $PLUGINDIR/formatting.pls $USERHEAD $DBLINKHEAD $WTEVTHEAD $WAITHEAD $ENQHEAD $SPADVHEAD $RSRCHEAD $PROFHEAD $TSQUOTHEAD $FILESHEAD $RBSHEAD $MEMVALHEAD $POOLHEAD $BUFFRATHEAD $FLCHEAD $INVOBJHEAD $MISCHEAD $BINDIR/repopen.pls $USERBODY $DBLINKBODY $RSRCBODY $PROFBODY $TSQUOTBODY $FILESBODY $RBSBODY $MEMVALBODY $POOLBODY $TMPADV $SYSSTAT $WTEVTBODY $BUFFRATBODY $FLCBODY $WAITBODY $ENQBODY $INVOBJBODY $MISCBODY $BINDIR/repclose.pls >rep.out
cat $SQLSET $BINDIR/rephead.pls $PLUGINDIR/formatting.pls $USERHEAD $DBLINKHEAD $WTEVTHEAD $WAITHEAD $ENQHEAD $SPADVHEAD $RSRCHEAD $PROFHEAD $TSQUOTHEAD $FILESHEAD $RBSHEAD $MEMVALHEAD $POOLHEAD $BUFFRATHEAD $FLCHEAD $INVOBJHEAD $MISCHEAD $BINDIR/repopen.pls $USERBODY $DBLINKBODY $RSRCBODY $PROFBODY $TSQUOTBODY $FILESBODY $RBSBODY $MEMVALBODY $POOLBODY $TMPADV $SYSSTAT $WTEVTBODY $BUFFRATBODY $FLCBODY $WAITBODY $ENQBODY $INVOBJBODY $MISCBODY $BINDIR/repclose.pls | $ORACLE_HOME/bin/sqlplus -s /NOLOG
